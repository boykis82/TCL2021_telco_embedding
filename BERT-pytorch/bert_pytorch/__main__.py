...
def train():
    ...
    if args.train_ratio < 1.0:
        ...
        test_data_loader = DataLoader(test_dataset, batch_size=args.batch_size, num_workers=args.num_workers) if args.train_ratio < 1.0 else None
        print(f'Create Dataloader. train = {len(train_data_loader)}, test = {len(test_data_loader)}')
    else:
        print(f'Dataset loading completed! train size = {len(train_dataset)}')
        print(f'Create Dataloader. train = {len(train_data_loader)}')
