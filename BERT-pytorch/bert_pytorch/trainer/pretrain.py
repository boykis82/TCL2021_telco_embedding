...
class BERTTrainer:
    ...
    def __init__(self, bert: ALBERT, embed_size:int, vocab,
                 ...
                 lr: float = 1e-3, betas=(0.9, 0.999), weight_decay: float = 0.01, warmup_steps=10000,
        ...
        self.mlm_criterion = nn.CrossEntropyLoss(ignore_index=0, reduction='none')
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in enumerate(data_loader):
            ...
            sent_order_output, mask_lm_output = self.model.forward(data["bert_input"], data["segment_label"])
            ...
            #    mask_lm_output = B * seq_len * vocab_size            
            mlm_loss = self.mlm_criterion(mask_lm_output.transpose(1, 2), data["bert_label"]).mean()
            ...
            correct = sent_order_output.argmax(dim=-1).eq(data["ordered"]).sum().item()
            sum_loss += loss.item()
            total_correct += correct
            total_element += data["ordered"].nelement()

            sop_acc = total_correct / total_element * 100
            ...
            self.summary_writer.add_scalar('accuracy', sop_acc, global_step)       

            if i % self.log_freq == 0:
                print(post_fix)
                self.summary_writer.flush()    

        print("EP%d_%s, avg_loss=" % (epoch, str_code), sum_loss / len(data_loader), "total_acc=",
    ...
    def load(self, ckpt_file_path):
        self.bert = torch.load(ckpt_file_path)
