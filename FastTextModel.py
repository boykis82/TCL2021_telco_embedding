...
class FastTextModel(object):
    ...
    def load(self, model_file_name):
        # load
        self.gensim_ft_model = FastText.load(model_file_name)
        # 후처리
    ...
    def create(self, in_corpus_file_name, out_model_file_name,
        ...
        # 빈도수 상위 n위의 최소빈도수 구함 (fasttext 훈련 시 그 이하는 버리기 위함)
        ...
        self.gensim_ft_model = FastText(vector_size=embedding_size)

        self.gensim_ft_model.build_vocab(corpus_file=in_corpus_file_name)

        loss_logger = ll.LossLogger()        
            
        self.gensim_ft_model.train(
            corpus_file=in_corpus_file_name,
            epochs=epochs,
            total_examples=self.gensim_ft_model.corpus_count,
            total_words=self.gensim_ft_model.corpus_total_words,
        ...
        # 저장
        self.gensim_ft_model.callbacks = None
        self.gensim_ft_model.save(out_model_file_name)   
        # 후처리
        self._post_model_created()        
        
    def _post_model_created(self):
        self.vocab_size = self.gensim_ft_model.wv.vectors.shape[0]
        self.embedding_size = self.gensim_ft_model.wv.vectors.shape[1]
        #self.index2word = self.gensim_ft_model.wv.index2word
        self.index2word = self.gensim_ft_model.wv.index_to_key       

        # unk, pad 추가
        self.index2word.append( UNK_TOKEN )
        self.index2word.append( PAD_TOKEN )    
        
        self.weight = self.gensim_ft_model.wv.vectors
